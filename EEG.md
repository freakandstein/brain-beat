...
| **Double jaw clench detection** | ✅ Selesai | `GestureComposer` menghitung rising edge jaw clench dalam window 1.5s (`DECIDE_DELAY`, diukur dari release) — 2+ edge → double_jaw, 1 edge → single jaw_clench — Command D playground |
...
| **Wink unilateral range widened** | ✅ Selesai | Weak-side range 10–300µV → 1–400µV — log nyata nunjukin wink kiri (AF7 dominan) sering gagal karena AF8 jatuh ke ~0µV (dianggap dropout) atau >300µV (dianggap dekat-eyebrow) |
| **Eyebrow sustained tuning** | ✅ Selesai | 3-tick-strict (gagal total) → 2-tick (kena microexpression ~300ms) → 3-tick + toleransi 1 tick noise (final) — lihat detail di BRAINWAVE_MONITOR.md |
...
| **Cursor Control Mode** | ✅ Selesai | Head-tilt joystick pakai accelerometer/gyroscope Muse 2 — kalibrasi arah eksplisit 3-tahap (neutral/kanan/atas) + ortogonalisasi Gram-Schmidt, jaw clench jadi left-click saat mode ON (mutual exclusion dengan scene switch), double jaw tetap toggle recording di kedua mode. Mutually exclusive dengan tilt_left/tilt_right (sensor IMU yang sama). Detail lengkap di BRAINWAVE_MONITOR.md |
| **Tilt left/right detection** | ✅ Selesai | Gesture roll kepala (telinga→bahu, quick tilt-and-release) dari accelerometer+gyroscope — kalibrasi multi-sample (3 percobaan dirata-rata + cek konsistensi arah, tanpa batas waktu), axis-dominance guard (tolak menoleh/mengangguk), guard arah gerakan (tolak kembali-ke-netral), release relatif ke puncak gesture, re-arm gate + auto-recenter baseline. Command E/F playground, `on_tilt_left`/`on_tilt_right`. Hanya aktif saat Cursor Control Mode OFF. Detail lengkap (8 iterasi bugfix) di BRAINWAVE_MONITOR.md |
| **Global mutex antar detector — extended ke tilt** | ✅ Selesai | Tilt fire ikut `_last_cmd_time` mutex yang sama dengan wink/jaw/eyebrow (tilt kuat berisiko menggoyang elektroda → artefak EMG palsu) |
...
| **Fase 4h** | ✅ Selesai | Tilt left/right (Command E/F) — gesture roll kepala pakai ACC/GYRO yang sama dengan Cursor Control Mode (mutually exclusive). Melalui ~8 iterasi bugfix dari log real-device: kalibrasi 1-sample → multi-sample rata-rata + cek konsistensi arah, pemilihan axis dominan dari titik gyro maksimum (bukan titik accel maksimum, yang ternyata di posisi kepala BERHENTI berputar), guard arah gerakan (tolak sinyal yang sedang kembali ke netral), release relatif terhadap puncak gesture (bukan ambang absolut), re-arm gate berbasis sinyal + auto-recenter baseline. Detail lengkap di BRAINWAVE_MONITOR.md |
